...
//1、数据发送采用可变长度，文件头以 0xFF,0x55开始，结束时CHKSUM校验位
//2、数据长度以第二个字节为准，不包括本身及CHKSUM，如下所示【0x01,0xA1】为要发送数据
...
u8 wifi_uart_rx_buf[8] = {0};///__attribute__((aligned(4)));		//接收数据暂存数组
u8 wifi_uart_tx_buf[8] = {0};///__attribute__((aligned(4)));		//发送数据暂存数组
