...
const u8 wifi_Effect_play[24] = {0, 3, 2, 3, 1, 2, 0, 3, 2, 3, 1, 3, 2, 3, 2, 1, 3, 2, 3, 3, 2, 1, 0, 3};
...
static u8 music_play_energy_lever(u8 *val)
{
	...
	if(!strcmp(keymsg_task_name,"WiFiTask"))
	{
		...
		lever = wifi_Effect_play[wifi_cnt];
	}
	else
	{
		lever = GetMusicEnergyLevel();
	}
	if(lever > 3)
		lever = 3;
	/* printf("lever = %d\n", lever); */
