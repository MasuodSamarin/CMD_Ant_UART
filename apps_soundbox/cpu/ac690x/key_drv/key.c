    ...
    if(KEY_TYPE_AD == key_type)
    {
        ...
        else
        {
            ...
            if(key_status == 0)
           {
                ...
                /* os_taskq_post_msg(menu_keytask_name, 1, msg); */
				if((key_lock_flag == 0)&&(wifi_power_on_start_statu == 1)) //儿童锁
				{
					key_msg_sender(menu_keytask_name, msg);
				}
           }
...
//    key_printf("msg:0x%02x\n",msg);
...
#endif
    {
		...
		if(((key_lock_flag == 0)||(msg == MSG_KEY_LOCK))&&(wifi_power_on_start_statu == 1))//儿童锁
		{
...
#if KEY_DOUBLE_CLICK
    if((!strcmp(keymsg_task_name,"btmsg")) && (key_lock_flag == 0))//蓝牙模式才有按键双击功能
    {
